...
Freq_adaptive_design <- function(surdata,
                                 ...
                                 p_int = rep(0.01, length(check)),							   # a vector of minimum p-value cutoffs for the treatment-by-marker interaction to decide whether to dichotomize (length=number of checks)
                                 p_eff_all = rep(0.01, length(check)),  					 # a vector of overall cohort one-sided log-rank test p-value cutoffs to claim efficacy (length=number of checks)
                                 p_eff_grp = rep(0.1, length(check)),						   # a vector of marker subgroup one-sided log-rank test p-value cutoffs to claim efficacy (length=number of checks)
                                 HR_fut_all = rep(1, length(check)-1),						 # a vector of overall cohort thresholds of HR above which to claim futility (length=number of checks - 1)
                                 HR_fut_grp = rep(1, length(check)-1),						 # a vector of marker subgroup thresholds of HR above which to claim futility (length=number of checks - 1)
