...
Sim_Shell_Exponential <- function(Nrep = 1, seed = 12342423,		            # Number of iterations and seed to reproduce results
   ...
   for (i in 1:Nrep) { 
      ...
      ##### Run Bayesian design #####
	    res.Bayes <- Bayes_adaptive_design_v3(surdata=res.data$surdata, ref=res.data$ref, n_events=n_events, m.prev.lwr=m.prev.lwr, check=check, marker.specs=marker.specs,
	    ...
	    ##### Run Freq design #####	
	    res.freq <- Freq_adaptive_design_v3(surdata=res.data$surdata, n_events=n_events, m.prev.lwr=m.prev.lwr, check=check, m.increment=m.increment, p_int=p_int_freq,
