...
# Compared to Bayes_adaptive_design, the major difference is that right censoring is allowed in the observed data, which is indicated by surdata$status #
###################################################################
         
            
BCM_data_application <- function(surdata,
				                        #####################################################################################################	 	
			                          ### surdata = a data frame with the observations arranged by row, and including the following columns:
				                        ...
				                        X,                                  # a data frame giving continuous marker values, with rows as patients and columns as biomarkers 
				     	                  n_events = sum(surdata$status),			# the number of events expected to be observed at the end of the trial
				     	                  m.prev.lwr = 0.2,						   		  # the lower bound allowed for the marker prevalence estimated from the splines fit
                                ...
                                alpha = 0.1,                        # the significance level at which an (1-alpha)*100% simultaneous/pointwise credible band is calculated
				     	                  p_eff_all = rep(0.95, length(check)),  					  # a vector of overall cohort posterior thresholds to claim efficacy (length=number of checks)
				     	                  p_eff_grp = rep(0.95, length(check)),  						# a vector of marker subgroup posterior thresholds to claim efficacy (length=number of checks)
				    	                  p_fut_all = rep(0.05, length(check)-1),						# a vector of overall cohort posterior thresholds for early stopping due to futility (length=number of checks - 1)
				    	                  p_fut_grp = rep(0.05, length(check)-1),						# a vector of marker subgroup posterior thresholds for early stopping due to futility (length=number of checks - 1)
         ...
         ###### Add estimation / interim decision code here ##### 
	       if(!stop.any) {
        	...
        	#### Time to Event Data T ####
         	...
         	result.j = Bayes_penalized_splines(data=data.j[,c("futime","status","trt")], X=X.j, xrange=t(apply(as.matrix(X), 2, range)), 
         	                                   ...
         	                                   censor=censor, MCMC.specs=MCMC.specs, alpha=alpha)
		      ...
		      if(!result.j$success) {
         	  ...
         	  ##### Check whether to dichotomize the marker based on the splines fit #####
			      fit.diff <- data.frame(est=result.j$est_diff, upr=result.j$upr_diff, lwr=result.j$lwr_diff, upr_pt=result.j$upr_diff_pt, lwr_pt=result.j$lwr_diff_pt)

				
			      ##### Loop over all subjects to determine marker status based on the dichotomization rule from the splines fit #####
			      marker <- rep(NA, nrow(X))
			      marker.j <- rep(NA, nrow(X.j))

			      if(any(fit.diff$upr_pt<0)) {

				      # define marker positivity
			        marker_pos <- X.j[fit.diff$upr_pt<0, , drop=FALSE]
			        marker_neg <- X.j[fit.diff$upr_pt>=0, , drop=FALSE]

				      if(nrow(marker_neg)==0) {
				        marker <- rep(1, nrow(X))
				        marker.j <- rep(1, nrow(X.j))
				      }

				      else {
				        for(k in 1:nrow(X)) {  
				          marker[k] <- as.numeric(min(dist_marker(X[k,], marker_pos)) <= min(dist_marker(X[k,], marker_neg)))
				        }
				        
				        for(k in 1:nrow(X.j)) {
				          marker.j[k] <- as.numeric(min(dist_marker(X.j[k,], marker_pos)) <= min(dist_marker(X.j[k,], marker_neg)))
				        }
           		}
			      }
					      ...
					      if(stop.eff.pos[j]==0 | stop.eff.neg[j]==0) {				  # if either marker cohort fails to stop early for efficacy, check futility
					        enter.rem <- enter[-index]
						      ...
						      for(l in 1:2000) {
							      ...
							      T.fut.pos.exp[data.pos.exp$status==0] <- data.pos.exp$futime[data.pos.exp$status==0] + rgamma(sum(data.pos.exp$status==0), shape=1, rate=hazard.pos.exp[l])
							      T.fut.pos.exp <- c(T.fut.pos.exp, rgamma(n.rem.pos.exp, shape=1, rate=hazard.pos.exp[l]))

							      # pos + ctl
							      T.fut.pos.ctl <- data.pos.ctl$futime
							      T.fut.pos.ctl[data.pos.ctl$status==0] <- data.pos.ctl$futime[data.pos.ctl$status==0] + rgamma(sum(data.pos.ctl$status==0), shape=1, rate=hazard.pos.ctl[l])
							      T.fut.pos.ctl <- c(T.fut.pos.ctl, rgamma(n.rem.pos.ctl, shape=1, rate=hazard.pos.ctl[l]))

							      # neg + exp
							      T.fut.neg.exp <- data.neg.exp$futime
							      T.fut.neg.exp[data.neg.exp$status==0] <- data.neg.exp$futime[data.neg.exp$status==0] + rgamma(sum(data.neg.exp$status==0), shape=1, rate=hazard.neg.exp[l])
							      T.fut.neg.exp <- c(T.fut.neg.exp, rgamma(n.rem.neg.exp, shape=1, rate=hazard.neg.exp[l]))

							      # neg + ctl
							      T.fut.neg.ctl <- data.neg.ctl$futime
							      T.fut.neg.ctl[data.neg.ctl$status==0] <- data.neg.ctl$futime[data.neg.ctl$status==0] + rgamma(sum(data.neg.ctl$status==0), shape=1, rate=hazard.neg.ctl[l])
							      ...
							      if(mean(hazard.fut.pos.exp < hazard.fut.pos.ctl) > p_eff_grp[j]) {
								      win.pos <- win.pos + 1
							      }

							      # Determine if the trial is a success for the negative cohort
							      if(mean(hazard.fut.neg.exp < hazard.fut.neg.ctl) > p_eff_grp[j]) {
								      win.neg <- win.neg + 1
							      }
						      }
				      ...
				      ##### If this is not the final check, perform efficacy and futility analysis #####	
					      ...
					      if(stop_eff & (eff.all >= p_eff_all[j])) {				# stop the overall cohort due to efficacy
						      ...
						      for(l in 1:2000) {
							      ...
							      if(mean(hazard.fut.exp < hazard.fut.ctl) > p_eff_all[j]) {
								      win <- win + 1
							      }
						      }
	         ...
	         # extract the marker negative cohort based on the dichotomization rule
		       ...
		       status.neg <- status[marker==0]
		       T.date.neg <- T.date[marker==0]


         	 #### Censor data appropriately #### 
		       event.date.neg <- T.date.neg[status.neg==1]       
         	 date <- sort(event.date.neg)[pmin(ceiling(n_events*mean(marker==0)*check[j]), length(event.date.neg))]
        	 ...
        	 #### Time to Event Data T ####
         	 enter.j <- enter.neg[index]			  		    # entry dates of those accured by current date
		       status.j <- status.neg[index]					    # event flag status (1=event, 0=censor) of those accrued by current date
         	 T.date.j <- T.date.neg[index]           	  # event or censor dates of those accrued by current date
         	 T.j <- T.date.j - enter.j		      		    # event or censor times of those accrued by current date
           ...
           ###### If this is the final check, skip the futility analysis ######
	         ...
	         # extract the marker positive cohort based on the dichotomization rule
		       ...
		       status.pos <- status[marker==1]
		       T.date.pos <- T.date[marker==1]


         	 #### Censor data appropriately #### 
		       event.date.pos <- T.date.pos[status.pos==1]         
         	 date <- sort(event.date.pos)[pmin(ceiling(n_events*mean(marker==1)*check[j]), length(event.date.pos))]
        	 ...
        	 #### Time to Event Data T ####
         	 enter.j <- enter.pos[index]			  		           # entry dates of those accured by current date
		       status.j <- status.pos[index]					           # event flag status (1=event, 0=censor) of those accrued by current date
         	 T.date.j <- T.date.pos[index]           	     		 # event or censor dates of those accrued by current date
         	 T.j <- T.date.j - enter.j		      		           # event or censor times of those accrued by current date
	    ...
	    return(list(stop.eff.pos=stop.eff.pos, stop.eff.neg=stop.eff.neg, stop.eff.all=stop.eff.all, 
                  ...
                  continue.pos=continue.pos, continue.neg=continue.neg, continue.all=continue.all, res.final=res.final,
	                sampsize=sampsize, sampsize.pos=sampsize.pos, sampsize.neg=sampsize.neg, 
			            marker.prevalence=marker.prevalence, X=X, marker.status=marker.status, fail.MCMC=FALSE))   
