...
# n = 500, threshold for marker dichotomization = 0.10, one interim check
...
stop.eff.all.Bayes <- stop.eff.all.Bayes[idx,]
stop.eff.pos.Bayes <- stop.eff.pos.Bayes[idx,]
stop.eff.neg.Bayes <- stop.eff.neg.Bayes[idx,]
stop.fut.all.Bayes <- matrix(stop.fut.all.Bayes[idx,], ncol=1)
stop.fut.pos.Bayes <- matrix(stop.fut.pos.Bayes[idx,], ncol=1)
stop.fut.neg.Bayes <- matrix(stop.fut.neg.Bayes[idx,], ncol=1)
continue.all.Bayes <- matrix(continue.all.Bayes[idx,], ncol=1)
continue.pos.Bayes <- matrix(continue.pos.Bayes[idx,], ncol=1)
continue.neg.Bayes <- matrix(continue.neg.Bayes[idx,], ncol=1)
sampsize.Bayes <- sampsize.Bayes[idx]
sampsize.pos.Bayes <- sampsize.pos.Bayes[idx]
sampsize.neg.Bayes <- sampsize.neg.Bayes[idx]
prev.Bayes <- prev.Bayes[idx]
...
for(i in 1:length(final.Bayes)) {
	if(final.Bayes[i]=="eff_no") {
		TNR1.all.Bayes[i] <- 1
		TNR2.all.Bayes[i] <- 1
	}
	else if(final.Bayes[i]=="eff_all") {
		TNR1.all.Bayes[i] <- 0
		TNR2.all.Bayes[i] <- 0
	}
	else {
		x.Bayes <- marker.value.Bayes[i,]
		idx1.Bayes <- which( unlist(lapply(x.Bayes,fnc.trt)) > log(0.95) )
		idx2.Bayes <- which( unlist(lapply(x.Bayes,fnc.trt)) > log(0.99) )

			if(final.Bayes[i]=="eff_pos") {
				TNR1.sub.Bayes[i] <- mean(marker.status.Bayes[i,idx1.Bayes]==0)
				TNR2.sub.Bayes[i] <- mean(marker.status.Bayes[i,idx2.Bayes]==0)
			}
			else if (final.Bayes[i]=="eff_neg") {
				TNR1.sub.Bayes[i] <- mean(marker.status.Bayes[i,idx1.Bayes]==1)
				TNR2.sub.Bayes[i] <- mean(marker.status.Bayes[i,idx2.Bayes]==1)
			}				
	}
}
...
for(i in 1:1000) {
	if(final.freq[i]=="eff_no") {
		TNR1.all.freq[i] <- 1
		TNR2.all.freq[i] <- 1
	}
	else if(final.freq[i]=="eff_all") {
		TNR1.all.freq[i] <- 0
		TNR2.all.freq[i] <- 0
	}
	else {
		x.freq <- marker.value.freq[i,]
		idx1.freq <- which( unlist(lapply(x.freq,fnc.trt)) > log(0.95) )
		idx2.freq <- which( unlist(lapply(x.freq,fnc.trt)) > log(0.99) )

			if(final.freq[i]=="eff_pos") {
				TNR1.sub.freq[i] <- mean(marker.status.freq[i,idx1.freq]==0)
				TNR2.sub.freq[i] <- mean(marker.status.freq[i,idx2.freq]==0)
			}
			else if (final.freq[i]=="eff_neg") {
				TNR1.sub.freq[i] <- mean(marker.status.freq[i,idx1.freq]==1)
				TNR2.sub.freq[i] <- mean(marker.status.freq[i,idx2.freq]==1)
			}				
	}
}
...
\begin{tabular}{rrrrrrrrrrrrrrrrr}
 ...
 & Interim Bayes & Interim Freq & Final Bayes & Final Freq & Overall Bayes & Overall Freq & Sample Size Bayes & Sample Size Freq & TPR1 Bayes & TPR1 Freq & TPR2 Bayes & TPR2 Freq & TNR1 Bayes & TNR1 Freq & TNR2 Bayes & TNR2 Freq \\ 
  \midrule
No Marker Detected & 0.822 & 0.591 & 0.213 & 0.199 & 0.822 & 0.638 & 377 & 405 &  &  &  &  & 0.979 & 0.970 & 0.979 & 0.970 \\ 
  Overall Efficacy & 0.001 & 0.008 & 0.001 & 0.008 & 0.017 & 0.019 &  &  &  &  &  &  &  &  &  &  \\ 
  Overall Futility & 0.575 & 0.299 & 0.212 & 0.191 & 0.805 & 0.619 &  &  &  &  &  &  &  &  &  &  \\ 
  Overall Continue & 0.246 & 0.284 &  &  &  &  &  &  &  &  &  &  &  &  &  &  \\ 
  Marker Detected & 0.178 & 0.409 & 0.152 & 0.217 & 0.178 & 0.362 & 118 & 149 &  &  &  &  & 0.676 & 0.602 & 0.676 & 0.602 \\ 
  M+ Eff/M- Eff & 0.003 & 0.001 & 0.012 & 0.002 &  &  & 302 & 224 &  &  &  &  &  &  &  &  \\ 
  M+ Eff/M- Cont & 0.081 & 0.021 &  &  &  &  &  &  &  &  &  &  &  &  &  &  \\ 
  M+ Eff/M- Fut & 0.056 & 0.259 & 0.117 & 0.103 & 0.173 & 0.362 &  &  &  &  &  &  &  &  &  &  \\ 
  M+ Cont/M- Eff & 0.004 & 0.000 &  &  &  &  &  &  &  &  &  &  &  &  &  &  \\ 
  M+ Cont/M- Cont & 0.025 & 0.000 &  &  &  &  &  &  &  &  &  &  &  &  &  &  \\ 
  M+ Cont/M- Fut & 0.008 & 0.111 &  &  &  &  &  &  &  &  &  &  &  &  &  &  \\ 
  M+ Fut/M- Eff & 0.000 & 0.000 & 0.005 & 0.000 & 0.005 & 0.000 &  &  &  &  &  &  &  &  &  &  \\ 
  M+ Fut/M- Cont & 0.001 & 0.000 &  &  &  &  &  &  &  &  &  &  &  &  &  &  \\ 
  M+ Fut/M- Fut & 0.000 & 0.017 & 0.018 & 0.112 &  &  &  &  &  &  &  &  &  &  &  &  \\ 
   \bottomrule
\end{tabular}
\end{table}
